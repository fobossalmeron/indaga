...
export const WebsiteNav = () => {
  ...
  return (
    <nav className="text-accent fixed top-0 right-0 left-0 z-50 flex h-16 w-full items-center justify-center bg-white/10 px-5 backdrop-blur-lg sm:px-10 md:px-20">
      <div className="flex w-full max-w-[1240px] items-center justify-between">
        <Link href="/" className="transition-all">
          <Logo
            width={95}
            height={18}
            className={`animate-fadeSimple transition-all ${isHomepageOrNosotras ? "text-white" : "text-foreground"}`}
          />
        </Link>
        <nav className="animate-fadeSimple flex items-center gap-7 mix-blend-exclusion transition-all">
