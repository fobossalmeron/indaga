...
export function RouteCard({ route }: { route: Content.RouteDocument }) {
  ...
  return (
    <article className={`flex h-full w-full max-w-[469px] flex-col overflow-hidden rounded-2xl bg-white transition-all duration-300 ease-in-out ${borderClass}`}>
        ...
        <div className="mt-auto pt-4">
          <Fade delay={350} className="flex w-full">
            <Button asChild className="w-fit px-10">
              <Link href={`/rutas/${route.uid}`}>Ver ruta</Link>
            </Button>
          </Fade>
        </div>
      </div>
